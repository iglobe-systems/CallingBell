...
var TechLoginPage = /** @class */ (function () {
    ...
    TechLoginPage.prototype.login = function () {
        ...
        this.authService.techLogin(username, password, body).subscribe(function (data) {
            console.log(data);
            console.log(data['result'].login.session, data['result'].modules[16].id);
            if (data['success']) {
                _this.authService.storeTechData(data['result'].login.session, data['result'].modules[16].id, data['result'].login.userid);
                _this.alertService.presentToast('Logged in successfully');
                _this.navCtrl.navigateRoot('tech-home');
            }
            else {
                _this.alertService.presentToast(data['success']);
                return false;
            }
        });
    };
